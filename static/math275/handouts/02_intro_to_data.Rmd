...
information---the data.  Today, you will gain insight into public health
...
When you begin a new analysis project you should create a new R project on RStudio to keep your work organized. To do this

* Go to File -> New Project
* Select New Directory -> Empty Project (or New Project if you have recently downloaded R on your own computer)
* Enter a logical directory name (e.g., `hw01`) and choose where you want to create this folder (click the browse button for an interface to help with this)
* Click Create Project

This will create and open a new R project, which is simply an easy way to stay organized. If you need to return to this project after closing it, then you can go to File -> Open Project and select the `.Rproj` file corresponding to your analysis.


Now that you have created your project, you should create a `data` folder in this directory and save any necessary data sets to this folder.

Today we will work from an Rmarkdown template that I have created. You should download this file from Moodle and save it to your project folder. Once it is saved in the correct location, you can open the `.Rmd` file by clicking on the link to it in the Files pane of RStudio.


### Loading the data

We begin by loading the data set of 20,000 observations into the R workspace. I will assume that you have created an R project with a `data/` folder that contains the `cdc.csv` data file.
After after opening the R project, enter the following command.

```{r load-data, eval=FALSE}
cdc <- read.csv("data/cdc.csv")
```

The data set `cdc` that shows up in your workspace is a *data table*, with each
...
1.  How many cases are there in this data set?  How many variables?  For each 
    variable, identify its data type (e.g. categorical, quantitative).
...
summary: minimum, first quartile (25<sup>th</sup> percentile), median, mean, third quartile (75<sup>th</sup> percentile), and maximum. 
For `weight` this is
...
instance, to calculate the mean, median, interquartile range, variance, and standard deviation of `weight`, type 
...
then immediately applied the graphical function, `barplot`. This is an 
...
for `barplot`. The special symbol `<-` performs an *assignment*, taking the 
...
Another thing to notice is that the plot is hard to read since it lacks axis labels. We can add labels to the x- and y-axes by adding the `xlab` and `ylab` arguments (note that the desired labels are in quotes):

```{r eval=FALSE}
barplot(table(cdc$smoke100), xlab = "Smoking status", ylab = "Number of respondents")
```
...
smoked at least 100 cigarettes. The rows refer to gender. 
...
we'll discuss in the future. For now, the important thing is that we can carve
...
quantitative data.  Two common ways to visualize quantitative data are with boxplots 
and histograms.  We can construct a boxplot for a single variable with 
the following command.

```{r boxplot-height, eval=FALSE}
boxplot(cdc$height, ylab = "Height (in inches)")
```
...
boxplot(cdc$height ~ cdc$gender, ylab = "Height (in inches)", xlab = "Sex",
        main = "Distribution of height by sex")
```

The notation here is new. The `~` character can be read *versus* or 
*as a function of*. So we're asking R to give us a boxplots of heights where 
the groups are defined by gender. 

Notice also that a title was added to the plot using the `main` argument.
...
The following two lines first make a new object called `bmi` and then create 
boxplots of these values, defining groups by the variable `cdc$genhlth`.

```{r boxplot-bmi, eval=FALSE}
cdc <- transform(cdc, bmi = (weight / height^2) * 703)
boxplot(cdc$bmi ~ cdc$genhlth, xlab = "General health status", ylab = "BMI",
        main = "Distribution of BMI by general health status")
```
...
4.  What does this boxplot show? Pick another categorical variable from the 
...
hist(cdc$age, xlab = "Age", main = "Distribution of age")
```
...
hist(cdc$bmi, xlab = "BMI", main = "Distribution of BMI")
hist(cdc$bmi, breaks = 50, xlab = "BMI", main = "Distribution of BMI")
```
...
-   Using numerical summaries and a side-by-side boxplot, determine if men tend
...
This was adapted by Adam Loy from OpenIntro Statistics and is released under a 
